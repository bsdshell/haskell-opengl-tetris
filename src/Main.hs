...
mymain = do
    ...
    bool successfulInit exitFailure $ do
        ...
        maybe' mw (G.terminate >> exitFailure) $ \window -> do
            ...
            lt <- randomIntList 10 (1, 4) >>= \cx -> return $ randomVexListInt (0, 0) cx
...
moveSqureColor :: Color3 GLdouble -> IO ()
moveSqureColor color = do
 preservingMatrix $ do
   let w = 0.1
   let h = 0.1
   let x = 0.1 / 2
...
moveSqureColorX :: Color3 GLdouble -> Float -> IO ()
moveSqureColorX color e = do
 preservingMatrix $ do
   let vf = Vertex3 (-1.0) 1.0 0.0 :: (Vertex3 GLfloat)
   let vv = Vertex3 0.0 0.0 0.0 -: vf
   translate vv
   drawRectGridColor color [0] [0] e
...
--}
    ...
    pre ls3
    when True $ do  
      (index, isNext, currRef) <- readRefFrame2 refCount 100
      --                                                  | 
      --                                                  + -> speed, larger = slower
...
-- /Users/aaa/myfile/bitbucket/tmp/xx_8590.x
          ...
          modifyIORef refGlobal (\x -> x{boardMap_ = DM.insert (lv !! index) (x0, rc) (boardMap_ x)})
        
        preservingMatrix $ do
          bm <- readIORef refGlobal >>= return . boardMap_
          let ls = map (\(x, c) -> (x -: Vertex3 0 0 0, c)) $ map snd $ DM.toList bm
          mapM_ (\(x, c) -> do
                       preservingMatrix $ do
                         translate x
                         preservingMatrix $ do
