...
color1 = Color3 0 0.3 0.9 :: Color3 GLdouble
color2 = Color3 0.5 0.3 0.9 :: Color3 GLdouble
color3 = Color3 0.2 0.3 0.2 :: Color3 GLdouble
...
crossF v@(Vector3 x y z) v'@(Vector3 x' y' z') = isZero ? Nothing $ Just (Vector3 x0 y0 z0) 
  where
    eps = 1e-12 
    vs = [[x'], [y'], [z']]
    sk = skewVec v
    ...
    y0 = (head . tail) ls
    z0 = last ls
    isZero = abs x0 < eps && abs y0 < eps && abs z0 < eps
...
isPerpen (Vector3 x y z) (Vector3 x' y' z') = x*x' + y*y' + z*z' == 0
...
drawPrimitive' m c ls = do
    renderPrimitive m $ mapM_(\v -> do
        color c
        vertex v) ls
    
drawPrimitive2::PrimitiveMode -> Color3 GLdouble -> [Vertex3 GLdouble]->IO()
drawPrimitive2 m c ls = do
    renderPrimitive m $ mapM_(\v -> do
        color c
        vertex v) ls

drawPrimitiveX::PrimitiveMode -> [Vertex3 GLdouble] -> [Color3 GLdouble] ->IO()
drawPrimitiveX m ls lc = do
    renderPrimitive m $ mapM_(\(v, c) -> do
        color c
        vertex v) $ zip ls (join $ repeat lc)
...
circlePtD (Vertex3 x y z) r n =[let alpha = pi2*(rf k)/n' in Vertex3 (r'*sin(alpha) + x) (r'*cos(alpha) + y) z | k <- [0..n]]
        where
            r' = rf r
            n' = rf n
            pi2 = 2*pi::Double
...
intersectLineTri (p0, p1) q@(q0, q1, q2) = (isCol || isPara) ? Nothing $ Just vx 
  where
   eps = 1e-12 
...
listToVer ls = Vertex3 (head lt) ((head . tail) lt) (last lt)
  where
    lt = take 3 ls  
...
ptToLine3d p0 (q1, q2) = nr vr
  where
    ...
    vp = mx `multiVecL` v0  -- p0 project onto v12
...
ptInsideTri p0 (a, b, c) = (isSameVer || isOK, isOK ? rad $ -1)
           where
             eps = 1e-12 
             allTrue ls = foldr (\a b -> a && b) True ls
             hasTrue ls = foldr (\a b -> a || b ) False ls 
             verCheck v0 v1 = let s = fmap (\x -> abs x < eps) $ v0 - v1 in allTrue s
             isSameVer  = hasTrue $ map (\x -> verCheck p0 x) [a, b, c]
...
isPtOnInscribeCircle v (a, b, c) = case mcen of
                                       Just cen -> let k = distX cen v - distX cen a in (abs(k) < 1e-5, k)
...
{-|
   ...
   * Sunday, 01 September 2024 14:04 PDT
   * Update the algorithm to avoid floating point error,
   * The privious algorith has ERROR in Swift
   ...
   uu = u dot u
   vv = v dot v
   x = d < 0 ? negate (d*d /uu * vv) $ (d * d / uu * vv)
   ϕ = acos x

   * NOTE: Make sure to check the sign of d dot v

-}
-- cosVex3::(Fractional a) => Vertex3 a -> Vertex3 a -> Vertex3 a -> a 
cosVex3::(Floating a, Ord a) => Vertex3 a -> Vertex3 a -> Vertex3 a -> a 
cosVex3 p0 p1 p2 = acos x
                where
                    v10 = p1 -: p0
                    v12 = p1 -: p2
                    d = dot3ve v10 v12
...
angleThreePts p0 p1 p2 = acos $ d/(n1*n2)
                    where
                      ...
                      v10 = p1 -: p0  -- vector p1 -> p0
                      v12 = p1 -: p2  -- vector p1 -> p2
                      d = dot3ve v10 v12
                      n1 = nr v10
                      n2 = nr v12
...
combinePt m cx = map (zip (join $ repeat cx)) $ hg m
  where
    gg xs = map (\(a, b) -> zipWith (\x y -> [x, y]) a b) xs 
    fg x y = zip (init x) (tail y)
...
drawCubeQuadX px r = do
  preservingMatrix $ do
    translate (vec_ px) 
    drawCubeQuadD r

drawCubeQuadT :: (GLdouble, Vertex3 GLdouble) -> IO ()
drawCubeQuadT (r, p0) = do
  preservingMatrix $ do
    translate (vec_ p0) 
    drawCubeQuadD r
...
drawCubeQuadN4X :: (GLfloat, GLfloat, GLfloat) -> [[Color4 GLdouble]] -> GLdouble -> GLdouble -> IO ()
drawCubeQuadN4X (x, y, z) colorls mc alpha = do
  {-
  ...
  -}
  let nfaces = zip3 n colorls facesx
  mapM_
    ( \(n, [c1, c2, c3, c4], [v0, v1, v2, v3]) -> do
        renderPrimitive Quads $ do
          normal n
    ...
    nfaces
  where
    -- fmap (*x) (Color4 r g b a) = Color4 (r*x) (g*x) (b*x) (a*x)
