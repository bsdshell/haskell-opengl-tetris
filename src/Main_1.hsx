...
setTranVecDrawRectX::(Vector3 GLdouble) -> GlobalRef -> GlobalRef
setTranVecDrawRectX vec gf = GlobalRef{
                    str_ = s, 
                    cursor_ = cursor, 
                    xyzAxis_ = xyz, 
                    mousePressed_ = mp, 
...
{--
    drawRectX2 w refGlobal vex cursorPos
--} 
--    when True $ do
--        preservingMatrix $ do
--            (Vector3 x y z) <- getTranVecDrawRectX refGlobal
--            translate (Vector3 x y 0 :: Vector3 GLdouble)
--
--            vex <- getDrawRectX refGlobal
--            let v1 = fst vex 
--                v2 = snd vex 
--                in drawRectX w vex cursorPos
...
--        z1 = 0.0::GLfloat in drawRect ((Vertex3 x0 y0 z0), (Vertex3 x1 y1 z1))
    ...
    -- cursorPos <- readIORef refGlobal >>= \x -> return $ cursor_ x
...
--        x1 = x
--        y1 = x 
--        z1 = 0.0::GLfloat in drawRectX w ((Vertex3 x0 y0 z0), (Vertex3 x1 y1 z1)) cursorPos 
...
--    drawCurve (\x -> x^2) (-1.0, 1.0) blue 
--    drawCurve (\x -> x^4 + 3*x^2 + 2*x + 3) (-4.0, 4.0) red
--    drawCurve (\x -> 9*x^2 - 6*x + 6)       (-4.0, 4.0) green 
...
{--
    let vex = let v0 = Vertex3 0   (-0.6) 0
                  v1 = Vertex3 (-0.6) (-0.6) 0
                  v2 = Vertex3 (-0.5) (-0.1) 0
                  v3 = Vertex3 0.0 0.0 0
              in [v0, v1, v2, v3]
    drawRectFillColorClick w vex
    ...
    VU.mapM_ (\(exp, c) -> do
        ...
        fx i j = let i' = rf i
                     j' = rf j
                 in σ * i'
        fy::Int -> Int -> GLflo        fy i j = let i' = rf i
                     j' = rf j
                     n = 3
                 in r * cos(δ * i')
...
--}
    ...
    pre ls3
    -- drawSegments cyan (takeSegment ls3)
    ...
    -- drawSurfaceFromList cm
    -- cylinder 0.1
    -- drawSurfaceFromList cl
    -- drawSegmentFromTo green $ map (\x -> Vertex3 x 0 0) [0, 0.1..0.6]
    -- drawSegmentFromTo green $ curvePtK (\x -> 0) (0, 0.8) 40

    -- circle center = (0.2 0 0) radius = 0.2
    -- mapM_ (\x -> drawSegmentFromTo blue x) lssVex
    ...
    --                             currTime - oldTime < n
    --
    (index, isNext, currRef) <- readRefFrame2 refCount 50

    -- drawQuads [Vertex3 0.1 0.1 0.0, Vertex3 0.2 00.1 0.0, Vertex3 0.2 0.2 0.0, Vertex3 0.1 0.2 0.0]
    -- drawQuadsXZColor green [Vertex3 0.1 0.1 0.0, Vertex3 0.2 0.1 0.0, Vertex3 0.2 0.2 0.0, Vertex3 0.1 0.2 0.0]

    
    -- animation stuff
    -- when (isNext && count  <= 10) $ do
      -- preservingMatrix $ do
        -- translate (Vector3 (0.04*(fi count)) 0 0 :: Vector3 GLdouble)
        -- drawQuadsYZColor blue [Vertex3 0.1 0.1 0.0, Vertex3 0.2 0.1 0.0, Vertex3 0.2 0.2 0.0, Vertex3 0.1 0.2 0.0]
    -- else return ()

    -- when (isNext && count > 10 && count <= 20) $ do
      -- preservingMatrix $ do
        -- translate (Vector3 (0.04*10.0) (0.04*(fi $ count - 10)) 0 :: Vector3 GLdouble)
        -- drawQuadsYZColor blue [Vertex3 0.1 0.1 0.0, Vertex3 0.2 0.1 0.0, Vertex3 0.2 0.2 0.0, Vertex3 0.1 0.2 0.0]
    -- else return ()
    ...
    when (isNext && index < len movingPathVex) $ do
      preservingMatrix $ do
        -- let vx = throwUpPath !! inx
        let vx = movingPathVex !! index
        -- affine geometry: vertex - vertex => vector
        let v3 = vx -: Vertex3 0 0 0
        translate v3
        -- drawQuadsColor green [Vertex3 0.1 0.1 0.0, Vertex3 0.1 (-0.1) 0.0, Vertex3 (-0.1) (-0.1) 0.0, Vertex3 (-0.1) 0.1 0.0]
        -- drawCircle2 (Vertex3 0.0 0.0 0.0) 0.05
        GL.scale (0.5::GLdouble) 0.5 0.5
        drawCylinderX cylinderPath
    -- else return ()

    when (index >= len movingPathVex) $ do
      resetRefFrame refCount

    {--
    if (isNext && index < 101) then do
       let logStr = "index=" ++ (show index) ++ " index=" ++ (show index)
       logFileNoName [logStr]  -- /tmp/x.x
       logFileNoName (map show $ interpolate !! index)
       drawSegmentFromTo green $ interpolate !! index
      else do
       resetRefFrame refCount
    --}
{-|
    when (isNext && index < 101) $ do
       let logStr = "index=" ++ (show index) ++ " index=" ++ (show index)
       logFileNoName [logStr]  -- /tmp/x.x
       logFileNoName (map show $ interpolate !! index)
       drawSegmentFromTo green $ interpolate !! index
...
{-|
    if isNext && count < len curveSet then do
      let inx = (fi count - 1)
      drawSegmentFromTo red $ curveSet !! inx
      else return ()
-}

    -- let arcls = circleNArc (Vertex3 0.0 0.0 0.0) 0.2 10 (0, pi)
    -- lsd <- (randomDouble $ 3 * (len arcls)) >>= \x -> return $ listToTuple2Vex x
    -- drawArc (0.2, 0, 0) arcls
    -- drawDot (Vertex3 (-0.5) (-0.5) 0)
    -- drawSurfaceFromList cl
...
{-|
   ...
   Segments from convexHullAllSeg
...
{-|
    cirList
    [  (
        [ Vertex3 0.1 0.1 0.0                       <- triangle
        , Vertex3 0.25 0.34 0.0
        , Vertex3 1.3 0.12 0.0
        ]
    , Just
        ( Vertex3 0.70367366 ( -0.11042103 ) 0.0 )  <- center of circle
    , 0.63929564                                    <- radius of circle
    )]
-}    
    -- let circleSet = map (\[x,y,z] -> threePtCircle x y z) validTriangle
    -- fw "circleSet"
    -- pre circleSet
